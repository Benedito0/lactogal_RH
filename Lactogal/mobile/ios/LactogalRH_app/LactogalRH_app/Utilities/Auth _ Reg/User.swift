...
class Register {
    ...
    init(email: String, NIF: Int, senha: String, name: String, phonenumber: Int, iban: Int) {
        self.email = email
        self.senha = senha
        self.name = name
    ...
    func registerUser(completion: @escaping (AuthResult) -> ()) {
        ...
        Auth.auth().createUser(withEmail: self.email, password: self.senha) { (user, err) in
            ...
            let userValues = ["fullname":self.name, "NIF": self.NIF, "email":self.email, "phonenumber": self.phonenumber, "IBAN": self.iban] as [String : Any]
